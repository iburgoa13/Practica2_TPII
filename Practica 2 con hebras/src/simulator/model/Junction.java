...
public class Junction extends SimulatedObject {
    ...
    void advance(int time) throws VehicleException, RoadException, JunctionException {
        //me creo una lista que almacene una lista de vehiculos
        List<List<Vehicle>>p = new ArrayList<>();

        //si hay una carretera en verde añado aquellos vehiculos que esten para cruzar
        //si uno (moveFirst) o todos(moveAll)
        if(_currGreen!=-1){
            p.add(_dq.dequeue(_queue.get(_currGreen)));
        }

        /*
        Recorro la lista de los coches moviendolos a la siguiente carretera
        y los elimino de la queue en donde estan
         */
       for(int i = 0; i < p.size();i++){
           for(int j = 0;j < p.get(i).size();j++){
               p.get(i).get(j).moveToNextRoad();
               _queue.get(_currGreen).remove(i);
           }
       }
